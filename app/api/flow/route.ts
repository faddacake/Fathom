...
export async function GET(req: NextRequest) {
  ...
  const { data: user } = await db.from('users').select('id, platform_tier, api_tier').eq('clerk_id', userId).maybeSingle();
  ...
  let query = db.from('flow_cache').select('*').order('traded_at', { ascending: false }).limit(limit).range(offset, offset + limit - 1);
  ...
  return NextResponse.json({
    data,
    meta: { count: data?.length ?? 0, tier: platformTier, delayed: platformTier === 'free' },
  });
}
